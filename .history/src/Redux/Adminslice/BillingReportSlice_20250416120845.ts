...
// In BillingReportSlice.ts
export const createBillReport = createAsyncThunk
  any, // Return type (replace with actual return type)
  { startDate: string endDate: string }, // Parameter type
  { rejectValue: string }
>("createBillReport", async (formData, { rejectWithValue }) => {
  try {
    const response = await axios.post(
      `https://mindora-backend-beta-version-m0bk.onrender.com/api/billing-reports`, 
      formData
    );
