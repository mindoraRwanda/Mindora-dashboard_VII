...
export const getAllPatientOfTherapy = createAsyncThunk('getAllPatientOfTherapy',
    async (therapistId: string, { rejectWithValue }) => {
        ...
        try {
            const response = await axios.get(`https://mindora-backend-beta-version-m0bk.onrender.com/api/therapists/bhttps://mindora-backend-beta-version-m0bk.onrender.com/api/therapists/b9b2d023-145c-47ec-8627-8b1545aaaf64/patients/patients`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`
                }
            })
