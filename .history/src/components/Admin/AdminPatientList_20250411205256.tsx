...
export default function AdminPatientsList() {
 ...
 const formatTime = (isoString) => {
  ...
  <div className="text-sm text-gray-700">
    Showing {filteredPatient.length > 0 ? ((currentPage - 1) * itemsPerPage + 1) : 0} to {Math.min(currentPage * itemsPerPage, filteredtherapists.length)} of {filteredtherapists.length} therapists
  </div>
  <div className="flex items-center space-x-1">
    ...
    {Array.from({ length: Math.ceil(fi.length / itemsPerPage) }, (_, i) => i + 1).map(
      (pageNumber) => (
        <button
          key={pageNumber}
