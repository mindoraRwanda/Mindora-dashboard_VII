 ...
 const formatTime = (isoString:any) => {
  ...
  const handleExportExcel = () => {
    const simplifiedData = filteredPatient.map(patient => ({
      ...
      Email: patient.user.email,
      Gender: patient.personalInformation.gender,
      Age: patient.personalInformation.age.toString()
    }));
              ...
              {slots.map((slot) => (
                <div key={slot.id}>
                  <div className="flex mt-2 justify-between mx-5 ">
                    ...
                    <p className="text-md">Monday</p><h2 className="flex gap-2">Patient_name: <p>{slot.patient.user.firstName}.{slot.patient.user.lastName}</p></h2>
                  </div>
