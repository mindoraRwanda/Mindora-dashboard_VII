import { Button, Form, Input, Modal, Spin, message, Card, Statistic, Tooltip, DatePicker, Space, Empty } from "antd";
import { CheckCircle, Calendar, DollarSign, Clock, AlertCircle, FileText, PieChart } from "lucide-react";
...
export default function BillingReports() {
  ...
  const handleCreateReport = async (values) => {
    try {
      await dispatch(createBillReport({
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm">
      ...
      {/* Create Report Modal */}
      <Modal
        title={<h2 className="text-xl font-semibold">Create New Billing Report</h2>}
        open={openBillModal}
        onCancel={handleCloseBillModal}
        footer={null}
      >
        <Form layout="vertical" onFinish={handleCreateReport} className="mt-4">
          <Form.Item 
