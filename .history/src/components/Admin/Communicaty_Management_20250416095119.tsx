...
const [AllPost, setAllPost] = useState<Post[]>([]);
  ...
  const ShowUpdateModal = (community: Community, e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent triggering community selection
  ...
  const handleUpdateCommunity = async (values: number|string) => {
  ...
  const handleUpdateComment = async (values: number | string) => {
    try {
      setLoading(true);
      const formData = new FormData();
      ...
      const result = await dispatch(updateComment({ id: selectedComment.id, commentData: formData }) as any);
...
useEffect(() => {
  const fetchPosts = async () => {
    ...
    setLoading(true);
    try {
      const result=await dispatch(getPostReport(selectedTopic.id) as any);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mb-6">
          {communities.map((community) => (
            <Card
              ...
              actions={[
                <Tooltip title="Edit">
                  <Button type="text" icon={<BiEdit size={20} />} onClick={(event) => ShowUpdateModal(community, event)} />
                </Tooltip>,
