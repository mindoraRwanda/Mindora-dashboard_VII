...
const handleAppointmentChange = async (value:any) => {
  try{
    setLoading(true);
    const ReschedulerData={
    ...selectedChangedAppointment,
    appointmentId: selectedChangedAppointment?.appointmentId,
    actionBy: selectedChangedAppointment?.actionBy,
    newStartTime: value.newStartTime,
    newEndTime: value.newEndTime,
    reason: value.reason,
    action:value.action,
  }
  await dispatch(createReschedule(ReschedulerData)).unwrap();
  await dispatch(getAllAppointmentChanges()).unwrap();
