...
function AppointmentChange({ changes }: AppointmentChangeProps) {
...
const handleAppointmentChange = async (value:any) => {
  try{
    setLoading(true);
    if (!selectedChangedAppointment || !selectedChangedAppointment.appointmentId) {
    ...
    <div className="bg-white rounded-lg shadow-xl ">
      {loading ? (
        ...
        <div>
          {changes.map((change) => (
            <div
              key={change.appointmentId}
              className="bg-white rounded border p-4 my-2"
            >
              <div>
              <strong className="text-black mb-2">
                  { change.otherParticipants ?
                  
                  `${change.otherParticipant?.firstName} ${change.otherParticipant?.lastName}`:
                ...
                <Button
                  ...
                  onClick={()=>handleDelete(change.appointmentId!)}
                  disabled={loading}
                >
