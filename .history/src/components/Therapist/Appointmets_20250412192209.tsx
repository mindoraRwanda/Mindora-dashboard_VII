        ...
        <div className="bg-white px-6">
          ...
          {getDisplayedAppointments().map((appointment) => (
            ...
            >
              ...
              <div className={`italic m-5 p-2 p-2 bg-gray-100 ${appointment?.status === "Scheduled"? "text-blue-600": appointment?.status === "Rescheduled"? "text-green-500": appointment?.status === "canceled" ? "text-red-700": ""}`}>{appointment?.status}
              </div>
            </div>
            )
          ))}
