...
const renderContent=()=>{
  ...
  const handleUpdateApp = async (values: string|number) => {
    try {
      setLoading(true);
        ...
        <div className="bg-white px-6">
          ...
          {getDisplayedAppointments().map((appointment) => (
            ...
            >
              ...
              <div className={`italic m-5 p-2 rounded-lg bg-gray-100 ${appointment?.status === "Scheduled"? "text-blue-600": appointment?.status === "Rescheduled"? "text-green-500": appointment?.status === "canceled" ? "text-red-700": ""}`}>{appointment?.status}
              </div>
            </div>
            )
          ))}
