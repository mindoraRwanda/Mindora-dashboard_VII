...
import { getAllPatientOfTherapy, Invoice } from "../../Redux/Adminslice/PatientSlice";
import {
  deletePatient,
  updatePatient,
...
const patient = useSelector((state: RootState) => state.patients.data);
  ...
  useEffect(() => {
    const getAllPatients=async()=>{
      try {
        ...
        console.log("Retrieved therapistId:", therapistId);
       await dispatch(getAllPatientOfTherapy(therapistId));
      } catch (error) {
...
// this is for pagination
  ...
  const safeFilteredPatientLength = Array.isArray(filteredPatient) ? filteredPatient.length : 0;
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentPatients = Array.isArray(filteredPatient) 
  ? filteredPatient.slice(indexOfFirstItem, indexOfLastItem) 
  : [];
  const handleDelete = async (patientId:number|string) => {
    ...
    if (confirmed) {
      try {
       ...
       await dispatch(getAllPatientOfTherapy()); // Refresh the patient list
      } catch (error) {
        const errorMessage = (error as Error).message;
        message.error(`Failed to load users: ${errorMessage}`);
      }
...
const handleExportExcel = () => {
  ...
  const handleCreateInvoice=async()=>{
   try{
    ...
    form.resetFields();
    await dispatch(getAllPatientOfTherapy());
...
const handleUpdateInvoice = async (invoiceId: any) => {
              ...
              <tr key={patient.id} className="hover:bg-gray-100">
                ...
                <div className="flex items-center">
                    <div className="h-10 w-10 flex-shrink-0">
                      {patient.patients.user.profileImage ? (
                        <img
                          className="h-10 w-10 rounded-full object-cover"
                          src={patient.patients.user.profileImage}
                          alt=""
                        />
                      ) : (
                        <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center">
                          <span className="text-gray-500">{patient.patients.user.firstName?.charAt(0)}</span>
                        </div>
                      )}
                    </div>
                    <div className="ml-4">
                      <div className="text-sm font-medium text-gray-900">
                        {patient.patients.user.firstName} {patient.patients.user.lastName}
                      </div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-no-wrap  border-gray-500">
                  <div className="text-sm leading-5 text-gray-900">
                    {patient.patients.user.email}
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-no-wrap  border-gray-500">
                  <div className="text-sm leading-5 text-gray-900">
                    {patient.patients.user.phoneNumber}
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-no-wrap  border-gray-500">
                  <div className="text-sm leading-5 text-gray-900">
                    {patient.patients.personalInformation.gender}
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-no-wrap  border-gray-500">
                  <div className="text-sm leading-5 text-gray-900">
                    {patient.patients.personalInformation.age}
                  </div>
                </td>
                <td className="px-4 py-3 whitespace-no-wrap  border-gray-500">
                  <div className="text-sm leading-5 text-gray-900">
                    {patient.patients.medicalProfile.lastVisit}
                  </div>
                </td>
  ...
  <div className="text-sm text-gray-700">
    Showing {safeFilteredPatientLength  > 0 ? ((currentPage - 1) * itemsPerPage + 1) : 0} to {Math.min(currentPage * itemsPerPage, safeFilteredPatientLength )} of {safeFilteredPatientLength } Patients
  </div>
  <div className="flex items-center space-x-1">
    ...
    {Array.from({ length: Math.ceil(safeFilteredPatientLength  / itemsPerPage) }, (_, i) => i + 1).map(
      (pageNumber) => (
        <button
          key={pageNumber}
    ...
    <button 
      onClick={() => currentPage < Math.ceil(safeFilteredPatientLength  / itemsPerPage) && paginate(currentPage + 1)}
      disabled={currentPage === Math.ceil(safeFilteredPatientLength  / itemsPerPage)}
      className="px-3 py-1 rounded border border-gray-300 text-gray-700 hover:bg-gray-50 disabled:opacity-50"
    >
      Next
    </button>
