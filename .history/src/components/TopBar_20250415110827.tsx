...
// Define the proper interface for messages that matches your Redux state
...
export default function TopBar({ userRole, items = [] }: TopBarProps) {
  ...
  // Create properly typed menu items for Ant Design Dropdown
  const menuItems: ItemType[] = (messages as MessageItem[]).map((msg) => ({
    key: msg.id || String(Math.random()), // Ensure we always have a key, using random as fallback
    label: (
  ...
  // Fixed Calendar onChange handler to match the expected type
  ...
  // function to get name of logged in user stored at local storage
  useEffect(() => {
                ...
                <ul>
                  {menus.map((menu) =>
                    <li
                      ...
                      className="text-lg font-semibold text-red-500 flex justify-center cursor-pointer p-1 mt-1 gap-2 hover:bg-blue-100 rounded"><LogoutOutlined />{menu.label}</li>
                  )}
                </ul>
              </div>
            )}
