...
export default function TopBar({ userRole, items = [] }:TopBarProps) {
  ...
  const messages: Messages[] = useSelector((state: RootState) => state.messages.messages);
  ...
  const menuItems = messages.map((msg: Messages) => ({
    key: msg.id,
    label: (
  ...
  const handleCalendarChange  = (nextValue: Date) => {
    setValue(nextValue);
  };
