...
// Updated Message interface to match Redux state structure
interface Message {
  id?: string;
  sender: string;
  recipient: string;
...
// Interface for transformed message format used in UI
interface UIMessage {
  id?: string;
  sender: {
    firstName: string;
...
export default function TopBar({ userRole, items = [] }: TopBarProps) {
  ...
  const messageCount = useSelector((state: RootState) => state.messages.messageCount);

  // Transform messages to match the UI format expected by the component
  const messages: UIMessage[] = messagesFromStore.map(msg => ({
    id: msg.id,
    sender: {
      firstName: msg.sender.split(' ')[0] || '',
      lastName: msg.sender.split(' ')[1] || ''
    },
  ...
  // Fixed Calendar onChange handler with correct type signature
  const handleCalendarChange = (nextValue: Value, event: React.MouseEvent<HTMLButtonElement>) => {
