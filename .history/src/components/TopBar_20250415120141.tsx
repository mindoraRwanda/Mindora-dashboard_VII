...
import { Value } from 'react-calendar/dist/cjs/shared/types'; // Import the Value type from react-calendar
...
// Update the interface to match the actual structure from Redux
interface Message {
  id?: string;
  sender: {
    firstName: string;
  ...
  messageText: string;
  // Add any other fields that might exist in the Messages type from Redux
}
...
export default function TopBar({ userRole, items = [] }: TopBarProps) {
  ...
  const messages = useSelector((state: RootState) => state.messages.messages) as Message[];
  ...
  // Fix the onChange handler to match the expected type from react-calendar
